function selectGuard(guards) {
  let selectedGuard;
  let totalMinutes = 0;

  for (const guard in guards) {
    const minutesAsleep = guards[guard].sleepTimes.reduce(
      (total, min) => total + min,
      0
    );
    if (totalMinutes < minutesAsleep) {
      selectedGuard = guard;
      totalMinutes = minutesAsleep;
    }
  }

  const mostSleptMinuteIndex = Math.max(...guards[selectedGuard].hour);
  const mostSleptMinute = guards[selectedGuard].hour.findIndex(
    min => min === mostSleptMinuteIndex
  );

  return selectedGuard * mostSleptMinute;
}

function sneakIn(array) {
  const guards = {};
  let prevGuard = '';
  for (let i = 0; i < array.length; i++) {
    const currGuard = array[i].split(' ');
    const guardOrSleepStatus = currGuard[3];

    if (guardOrSleepStatus.startsWith('#')) {
      prevGuard = guardOrSleepStatus.substring(1);
      if (!guards[prevGuard]) {
        guards[prevGuard] = {
    ...
    } else if (guardOrSleepStatus.startsWith('a')) {
      const sleepStart = parseInt(array[i].split(' ')[1].slice(3, 5));
      const sleepEnd = parseInt(array[i + 1].split(' ')[1].slice(3, 5));
      const timeDiff = sleepEnd - sleepStart;
      guards[prevGuard].sleepTimes.push(timeDiff);
      for (let j = sleepStart; j <= sleepEnd; j++) {
        guards[prevGuard].hour[j]++;
      }
    }
  }
  return selectGuard(guards);
}

const exampleStr = `[1518-07-18 23:57] Guard #157 begins shift
