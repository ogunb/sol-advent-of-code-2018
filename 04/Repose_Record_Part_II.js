function checkMostFrequentMinute(guards) {
  const guardsByMinute = {};
  Object.keys(guards).forEach(guard => {
    guardsByMinute[guard] = guards[guard].hour;
  });

  let [selectedGuard, selectedMinute] = [0, 0];
  for (let i = 0; i < 60; i++) {
    /* eslint-disable guard-for-in */
    for (const guard in guards) {
      if (selectedMinute < guards[guard].hour[i]) {
        selectedGuard = guard;
        selectedMinute = i;
      }
    }
...
function sneakIn(array) {
  ...
  return checkMostFrequentMinute(guards);
}
